    ...
    for (auto &inimigo : inimigos)
    {
        ...
        if (inimigoLimites.intersects(proxPosicao))
        {
            if (colisaoDireita(jogadorLimites, inimigoLimites))
            {
                ...
                if(inimigo->getId() == 16)
                    jogador->diminuirPontos(100);
                else
                    jogador->diminuirPontos(20);
            }
            ...
            {
                inimigo->desenhavel.setPosition(0.f, inimigo->desenhavel.getPosition().y);
...
void GerenciadorColisoes::verificarColisaoChaoInimigo(RenderWindow &janela)
{
    ...
    for (auto &inimigo : inimigos)
    {
        if (inimigo->getGlobalBounds().top + inimigo->getGlobalBounds().height > janela.getSize().y)
        {
            ...
            inimigo->setPosition(inimigo->getGlobalBounds().left, janela.getSize().y - inimigo->getGlobalBounds().height);
        }
    }
}
...
{
    if (jogador->getPosition().x <= 600)
        jogador->setPosition(600, jogador->getPosition().y);
}

void GerenciadorColisoes::verificarColisaoInicioTelaRainha()
{
    Inimigo inimigo;
    for (auto &inimigo : inimigos)
    {
...
{
    verificarColisaoInicioTelaJogador(jogador);
    verificarColisaoInicioTelaRainha();
